...
public class ListaUsuariosController implements Initializable {
    ...
    private static final Logger log = Logger.getLogger(ListaUsuariosController.class.getName());
    ...
    public void initialize(URL url, ResourceBundle rb) {
        log.info("Inicializando ListaUsuariosController...");
        configurarTabla();
        cargarDatosTabla();
    ...
    private void configurarTabla() {
        log.info("Configurando columnas de la tabla y valores iniciales de ComboBox.");
    ...
    private void configurarSeleccionTabla() {
        tblUsuarios.getSelectionModel().selectedItemProperty().addListener((obs, oldSelection, newSelection) -> {
            if (newSelection != null) {
                ...
                log.info("Usuario seleccionado de la tabla: ID " + usuarioSeleccionado.getIdUsuario() + " | Username: " + usuarioSeleccionado.getNombreUsuario());
    ...
    public void cargarDatosTabla() {
        log.info("Cargando la lista completa de usuarios desde la base de datos.");
    ...
    public void cargarDatos(Usuario usuario) {
        log.info("Cargando datos del usuario seleccionado en los campos de texto.");
        if (txtID != null) {
    ...
    private void handleGuardarUsuario() {
        ...
        if (usuarioSeleccionado == null) {
            ...
            if (usuarioDAO.insertar(nuevoUsuario)) {
                log.info("Usuario guardado exitosamente: " + nuevoUsuario.getNombreUsuario());
                mostrarAlerta("Usuario creado correctamente.");
                handleLimpiarUsuario();
                cargarDatosTabla();
            } else {
                log.warning("Fallo al intentar registrar el usuario: " + nuevoUsuario.getNombreUsuario());
                mostrarError("No se pudo registrar el usuario.");
            }
        } else {
            log.info("Iniciando actualización del usuario ID: " + usuarioSeleccionado.getIdUsuario());
            ...
            if (usuarioDAO.actualizar(usuarioSeleccionado)) {
                log.info("Usuario actualizado exitosamente: " + usuarioSeleccionado.getNombreUsuario());
                mostrarAlerta("Usuario actualizado correctamente.");
                handleLimpiarUsuario();
                cargarDatosTabla();
            } else {
                log.warning("Fallo al actualizar el usuario ID: " + usuarioSeleccionado.getIdUsuario());
    ...
    private void handleEditarUsuario() {
        if (usuarioSeleccionado == null) {
            log.warning("Intento de edición sin haber seleccionado un usuario.");
            mostrarError("Por favor, seleccione un usuario de la tabla para editar.");
            return;
        }
        log.info("Ejecutando actualización explícita para el usuario ID: " + usuarioSeleccionado.getIdUsuario());
        boolean EstadoActivo = "Activo".equalsIgnoreCase(cmbEstado.getValue());
        usuarioSeleccionado.setNombreUsuario(txtUsuario.getText().trim());
        usuarioSeleccionado.setNombre(txtNombre.getText().trim());
        usuarioSeleccionado.setApellido(txtApellido.getText().trim());
        ...
        if (usuarioDAO.actualizar(usuarioSeleccionado)) {
            log.info("Usuario editado correctamente: " + usuarioSeleccionado.getNombreUsuario());
            mostrarAlerta("Usuario actualizado correctamente.");
            handleLimpiarUsuario();
            cargarDatosTabla();
        } else {
            log.warning("No se pudo actualizar el usuario ID: " + usuarioSeleccionado.getIdUsuario());
            mostrarError("No se pudo actualizar el usuario.");
        }
    }
    ...
    private void handleLimpiarUsuario() {
        log.info("Limpiando formulario y deseleccionando usuario de la tabla.");
    ...
    private void handleDesactivarUsuario() {
        if (usuarioSeleccionado == null) {
            log.warning("Intento de desactivación sin seleccionar un usuario.");
            mostrarError("Por favor, seleccione un usuario de la tabla para desactivar.");
            return;
        }
        log.info("Solicitando confirmación para desactivar usuario: " + usuarioSeleccionado.getNombreUsuario());
        ...
        if (respuesta.isPresent() && respuesta.get() == ButtonType.YES) {
            ...
            if (usuarioDAO.actualizar(usuarioSeleccionado)) {
                log.info("Usuario desactivado con éxito: " + usuarioSeleccionado.getNombreUsuario());
                mostrarAlerta("Usuario desactivado correctamente.");
                handleLimpiarUsuario();
                cargarDatosTabla();
            } else {
                log.warning("Error en BD al intentar desactivar usuario: " + usuarioSeleccionado.getNombreUsuario());
                mostrarError("No se pudo desactivar el usuario.");
            }
        } else {
            log.info("Desactivación cancelada por el usuario.");
        }
    }

    @FXML
    private void handleVolverMenu(ActionEvent event) {
        log.info("Intentando regresar al menú principal (DashboardAdminView.fxml).");
        try {
            ...
            log.log(Level.SEVERE, "Error al intentar volver al menú de administración", e);
            e.printStackTrace();
    ...
    private boolean validarCampos() {
        log.info("Ejecutando validación de campos del formulario de usuario.");
        ...
        if (errores.length() > 0) {
            log.warning("Validación de campos fallida:\n" + errores.toString());
    ...
    public void handleCambiarContrasena(ActionEvent event) {
        log.info("Navegando hacia la vista de cambio de contraseña (CambioContraseñaView.fxml).");
        try {
            ...
            log.log(Level.SEVERE, "Error de I/O al abrir la vista CambioContraseñaView.fxml", e);
            e.printStackTrace();
    ...
    private void mostrarError(String mensaje) {
        log.warning("Mostrando ventana de error: " + mensaje);
    ...
    private void mostrarAlerta(String mensaje) {
        log.info("Mostrando ventana de información: " + mensaje);
